...
const std::vector<double> example_point_RXSM{
    /* lambdaS = */ 0.34892332012937127,
    /* lambdaHS = */ -0.02992292362975825,
    /* vS = */ 751.21347663399922,
    /* MassS = */ -1}; // value of MassS should be irrelevant if vS =/= 0

const std::vector<double> example_point_RXSM_noVS{
    /* lambdaS = */ 0.34892332012937127,
    /* lambdaHS = */ -0.02992292362975825,
    /* vS = */ 0,
    /* MassS = */ 500.}; // value of MassS must be given since vS == 0

const Compare_RXSM Expected;
const Compare_RXSM_noVS Expected_noVS;

TEST_CASE("Checking NLOVEV for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
      ModelID::FChoose(ModelID::ModelIDs::RXSM, SMConstants);
  modelPointer->initModel(example_point_RXSM);
...
TEST_CASE("Checking EWPT for RXSM (vS =/= 0)", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking EWPT for RXSM (vS == 0)", "[RXSM]")
{
  ...
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
      ModelID::FChoose(ModelID::ModelIDs::RXSM, SMConstants);
  modelPointer->initModel(example_point_RXSM_noVS);
  std::vector<double> Check;
  auto EWPT = Minimizer::PTFinder_gen_all(
      modelPointer, 0, 300, Minimizer::WhichMinimizerDefault);
  const double omega_c_expected =
      Expected_noVS.EWPTPerSetting.at(Minimizer::WhichMinimizerDefault).vc;
  const double Tc_expected =
      Expected_noVS.EWPTPerSetting.at(Minimizer::WhichMinimizerDefault).Tc;
  const std::vector<double> min_expected =
      Expected_noVS.EWPTPerSetting.at(Minimizer::WhichMinimizerDefault)
          .EWMinimum;
  REQUIRE(EWPT.StatusFlag == Minimizer::MinimizerStatus::SUCCESS);
  REQUIRE(std::abs(EWPT.vc) == Approx(omega_c_expected).epsilon(1e-4));
  REQUIRE(EWPT.Tc == Approx(Tc_expected).epsilon(1e-4));
...
TEST_CASE("Checking number of CT parameters for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking number of VEV labels for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE(
    "Checking number of labels for temperature dependend results for RXSM",
    "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Checking number of triple Higgs couplings for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking Gauge Boson masses for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking fermion and quark masses masses for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking tree level minimum for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking tree level tadpoles for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking NLO masses matching tree level masses for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking VTreeSimplified for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking VCounterSimplified for RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Checking first derivative of the sum of CT and CW in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Checking second derivative of the sum of CT and CW in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Checking the identities required to vanish for the CT in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Checking triple higgs NLO couplings in the RXSM (vS =/= 0)",
          "[RXSM]")
{
...
TEST_CASE("Checking triple higgs NLO couplings in the RXSM (vS == 0)", "[RXSM]")
{
  ...
  for (std::size_t i{0}; i < NHiggs; ++i)
  {
    for (std::size_t j{0}; j < NHiggs; ++j)
    {
      for (std::size_t k{0}; k < NHiggs; ++k)
      {
        INFO("Checking TreePhysical");
        Check(modelPointer->get_TripleHiggsCorrectionsTreePhysical(i, j, k),
              Expected_noVS.CheckTripleTree.at(i).at(j).at(k));
        INFO("Checking CTPhysical");
        Check(modelPointer->get_TripleHiggsCorrectionsCTPhysical(i, j, k),
              Expected_noVS.CheckTripleCT.at(i).at(j).at(k));
        INFO("Checking CWPhysical");
        Check(modelPointer->get_TripleHiggsCorrectionsCWPhysical(i, j, k),
              Expected_noVS.CheckTripleCW.at(i).at(j).at(k));
      }
    }
  }
}

TEST_CASE("Check number of calculated CT parameters in the RXSM", "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
  std::shared_ptr<BSMPT::Class_Potential_Origin> modelPointer =
...
TEST_CASE("Check symmetric properties of the scalar tensor Lij in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the scalar tensor Lijk in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the scalar tensor Lijkl in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the gauge tensor in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the Lepton tensor in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the mass Lepton tensor in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the mass quark tensor in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
...
TEST_CASE("Check symmetric properties of the quark tensor in the RXSM",
          "[RXSM]")
{
  using namespace BSMPT;
  const auto SMConstants = GetSMConstants();
