...
struct CLIOptions
{
  ...
  std::vector<double> UserDefined_vwall = {0.95};
          ...
          (output.status.status_coex_pairs == StatusCoexPair::Success))
      {
             ...
             i++)
        {
          ...
          const double vwall_with_fallback =
              (args.UserDefined_vwall.at(0) < 0 and not input.only_crit)
...
bool CLIOptions::good() const
{
  ...
  for (const auto &vwall : UserDefined_vwall)
    if ((vwall != -1 and vwall != -2) and (vwall <= 0 or vwall > 1))
    {
...
CLIOptions::CLIOptions(const BSMPT::parser &argparser)
{
  ...
  try
  {
    auto vwall_str = split(argparser.get_value("vwall"), ',');
    UserDefined_vwall.clear();
    for (std::size_t i = 0; i < vwall_str.size(); i++)
    {
      double vwall = std::stod(vwall_str.at(i));
      UserDefined_vwall.push_back(vwall); // check if 2 + 4n
    }
    // sort and remove duplicates
    std::sort(UserDefined_vwall.begin(), UserDefined_vwall.end());
    UserDefined_vwall.erase(
        unique(UserDefined_vwall.begin(), UserDefined_vwall.end()),
        UserDefined_vwall.end());
  }
  catch (BSMPT::parserException &)
  {
