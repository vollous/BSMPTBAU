...
template <class T> NRvector<T>::NRvector() : nn(0), v(nullptr)
{
}

template <class T>
NRvector<T>::NRvector(int n) : nn(n)
                             , v(n > 0 ? new T[n] : nullptr)
{
}

template <class T>
NRvector<T>::NRvector(int n, const T &a) : nn(n)
                                         , v(n > 0 ? new T[n] : nullptr)
{
  for (int i = 0; i < n; i++)
    v[i] = a;
...
public:
  ...
  void
  assign(int newn, int newm, const T &a); // resize and assign a constant value
  ~NRmatrix();
};

template <class T> NRmatrix<T>::NRmatrix() : nn(0), mm(0), v(nullptr)
{
}
...
template <class T> NRMat3d<T>::NRMat3d() : nn(0), mm(0), kk(0), v(nullptr)
{
}

template <class T>
NRMat3d<T>::NRMat3d(int n, int m, int k) : nn(n)
                                         , mm(m)
                                         , kk(k)
                                         , v(new T **[n])
{
